...
import 'package:mlrcc/core/core.dart'; // Application-specific code
import 'package:mlrcc/core/providers.dart'; // Application-specific code

/// Riverpod provider for the authentication API.
final authAPIProvider = Provider((ref) {
  final account = ref.watch(authProvider); // Get the Firebase Authentication instance from another provider
  return AuthAPI(
    auth: account, // Pass the Firebase Authentication instance to the AuthAPI constructor
  );
});
...
class AuthAPI implements IAuthAPI {
  final FirebaseAuth _auth; // Private instance of the Firebase Authentication service.
  ...
  FutureEither<UserCredential> signInWithGoogle(BuildContext context) async {
    try {
      final GoogleSignInAccount? googleUser = await GoogleSignIn().signIn(); // Get a GoogleSignInAccount

      final GoogleSignInAuthentication? googleAuth =
          await googleUser?.authentication; // Get the Google authentication tokens
      ...
      if (googleAuth?.accessToken != null && googleAuth?.idToken != null) {
        ...
        return left(Failure('Google Sign In Failed', StackTrace.empty)); // Return a Left with a Failure object if the sign-in fails
      }
    } on FirebaseAuthException catch (e, st) {
      return left(Failure(e.message ?? e.toString(), st)); // Return a Left with a Failure object if there is an authentication error
    }
  }
